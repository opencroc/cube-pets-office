...
为论文中的 18 个智能体编写初始 SOUL.md。可以从论文附录 A 的模板出发，每个文件包含：
...
就是 SOUL.md 文件本身。每次智能体调用时完整读入。进化阶段自动追加 `## Learned Behaviors` 章节。
